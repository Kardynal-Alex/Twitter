...
@Component({selector: 'app-view-users-tweet', templateUrl: './view-users-tweet.component.html', styleUrls: ['./view-users-tweet.component.css']})
export class ViewUsersTweetComponent implements OnInit {

    constructor(private twitterPostService : TwitterPostService, private router : Router, private authService : AuthService, private favoriteService : FavoriteService, private likeService : LikeService) {}
    ...
    changeIconSavedPost(twitterPostId : string) {
        document.getElementById("save-" + twitterPostId).style.background == "" ? this.addToFavorite(twitterPostId) : this.removeFromFavorite(twitterPostId);
    }
    ...
    changeHeartIconTweet(tweet : twitterPost) {
        document.getElementById("like-" + tweet['id']).style.background == "" ? this.likeTweet(tweet) : this.removeLikeTweet(tweet);
    }
