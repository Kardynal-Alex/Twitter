...
export class HeshtagSearchComponent implements OnInit {
    ...
    constructor(private twitterPostService : TwitterPostService, private favoriteService : FavoriteService, private router : Router, private authService : AuthService, private likeService : LikeService) {}
    ...
    searchTweets() {
        if (this.search != '') {
            this.twitterPostService.searchTweetsByHeshTag(this.search).subscribe(response => {
                ...
                setTimeout(() => this.renderfavoriteTweets(), 100);
