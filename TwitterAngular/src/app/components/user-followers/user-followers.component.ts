...
@Component({selector: 'app-user-followers', templateUrl: './user-followers.component.html', styleUrls: ['./user-followers.component.css']})
export class UserFollowersComponent implements OnInit {

    constructor(private friendService : FriendService, private authService : AuthService, private router : Router, private activateRoute : ActivatedRoute) {}

    ngOnInit() {
        document.getElementById('router').className = "router1";
        this.userId = this.activateRoute.snapshot.params['id'];
        if (this.userId) 
            this.seeUserFollowers(this.userId);
    ...
    seeUserFollowers(userId : string) {
        this.authService.getUserFollowers(userId).subscribe(response => {
            this.users = response;
            setTimeout(() => {
